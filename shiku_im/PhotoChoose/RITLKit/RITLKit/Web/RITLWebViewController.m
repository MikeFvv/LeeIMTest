...
@interface RITLWebViewController ()<WKScriptMessageHandler,WKUIDelegate>
...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        self.closeWidth = 32;
        self.leftImageInset = UIEdgeInsetsMake(0, 0, 0, 20);
...
- (void)viewDidLoad {
    ...
    if (self.p_hasNavigationBar) {
        ...
        if (self.useLeftCloseItem) {
            ...
            self.navigationItem.leftBarButtonItems = [self backItemsWithCanGoBack:true left:true];
        }
    }
...
-(void)viewDidLayoutSubviews
{
    ...
    if (self.p_hasNavigationBar) {
        ...
        self.webView.frame = CGRectMake(0, JX_SCREEN_TOP, self.ritl_width, self.ritl_height - JX_SCREEN_TOP);
    }
...
-(WKWebView *)webView
{
    if (!_webView) {
        ...
        _webView=[[WKWebView alloc]initWithFrame:CGRectMake(0, 0.5, kScreenWidth, kScreenHeight-0.5) configuration:self.configuration];
...
//{
    ...
    // 打开appstore
    if ([url.absoluteString containsString:@"https://itunes.apple.com/cn/app/"] || [url.absoluteString containsString:@"alipays://"] ||
        [url.absoluteString containsString:@"https://mclient.alipay.com"])
    {
