...
//            [self sendSelfMsg:_modifyType content:_content];
    ...
    if( [aDownload.action isEqualToString:act_roomDel] ){ //群组解散
...
//            [rData release];
    ...
    //
    if( [aDownload.action isEqualToString:act_roomMemberDel] ){ // 退出群组
...
//        [line release];
    ...
    //创建switch
    if(click == check){
        UISwitch * switchView = [[UISwitch alloc]initWithFrame:CGRectMake(JX_SCREEN_WIDTH-INSETS-51, (56 - 32) / 2, 60, 32)];
        ...
        UIImageView* iv;
        iv = [[UIImageView alloc] initWithFrame:CGRectMake(JX_SCREEN_WIDTH-INSETS-20-3, (56 - 20) / 2, 20, 20)];
...
//        membHeight+=self.iv.frame.size.height;
    ...
    _btn.frame = CGRectMake(20, membHeight, JX_SCREEN_WIDTH-40, 44);
...
-(void)setRoomframeWithHeight:(int)height{
    ...
    if(height+10 > JX_SCREEN_HEIGHT){
        self.tableBody.contentSize = CGSizeMake(JX_SCREEN_WIDTH, height-30);
    }else{
...
-(int)createImages{
    ...
    _heads = [[UIView alloc] initWithFrame:CGRectMake(0, 0, JX_SCREEN_WIDTH, 56)];
...
//        [g_server emptyMsgWithTouserId:self.roomId type:[NSNumber numberWithInt:2] toView:self];
