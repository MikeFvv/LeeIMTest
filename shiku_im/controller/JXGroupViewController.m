...
-(NSMutableArray *)allGroupArray{
    if (!_allGroupArray) {
        _allGroupArray = [NSMutableArray array];
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(user && [user.groupStatus intValue] == 0){
        ...
        _chatRoom = [[JXXMPP sharedInstance].roomPool joinRoom:dict.userId title:dict.userNickname isNew:NO];
        //老房间:
        [self showChatView];
    }else{
...
-(void) didServerResultSucces:(JXConnection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    ...
    if([aDownload.action isEqualToString:act_roomList] || [aDownload.action isEqualToString:act_roomListHis] ){
        ...
        for (int i = 0; i < [array1 count]; i++) {
            ...
            if (_selMenu == 0) {
                 [self.myGroupArray addObject:user];
               }else if (_selMenu == 1) {
                [self.allGroupArray addObject:user];
               }
