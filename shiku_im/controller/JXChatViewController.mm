...
@property (nonatomic, strong) UIButton *memuSetBtn;//右上角按钮

@property (nonatomic,copy) NSString *scanString;//识别的二维码
@end
...
- (id)init
{
    ...
    [g_notify addObserver:self selector:@selector(onUpdateMuteRemind:) name:kCellMuteRemindNotifaction object:nil];  // 控制消息点击
...
- (void)viewDidLoad
{
    ...
    // lewis 修改背景颜色
    self.view.backgroundColor = HEXCOLOR(0xF4F4F4);//HEXCOLOR(0xD0D0D0);
    ...
    if (self.courseId.length > 0) {
        ...
        UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(10,  JX_SCREEN_HEIGHT - (kDevice_Is_iPhoneX ? 34 : 0) - 56 - 20 , JX_SCREEN_WIDTH - 20, 56)];
...
//            syncTimeLen = [g_myself.groupChatSyncTimeLen doubleValue];
        ...
        if (syncTimeLen != -2) {
            ...
            if([self.roomJid length]>0) {
                ...
                [g_server tigaseMucMsgsWithRoomId:s StartTime:createTime EndTime:endTime PageIndex:0 PageSize:PAGECOUNT toView:self];
            }
            else
...
- (void) customView {
    ...
    if (imageData) {
        ...
        _table.backgroundColor = HEXCOLOR(0xF4F4F4);//HEXCOLOR(0xD0D0D0);
    }
...
//        _moreView.onLocation  = @selector(onLocation); //隐藏定位功能
...
- (void) createFooterSubViews{
    ...
    _messageText.layer.borderColor = HEXCOLOR(0x000000).CGColor;
    ...
    btn.layer.borderColor = [HEXCOLOR(0x000000) CGColor];
...
//                        [_table scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:(int)(_array.count - firstNum) inSection:0] atScrollPosition:UITableViewScrollPositionBottom animated:NO];
                        
                        _table.contentOffset = CGPointMake(0, allHeight);
                        
//                        [_table scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:_array.count inSection:0] atScrollPosition:UITableViewScrollPositionBottom animated:NO];
//                        _table.contentOffset = CGPointMake(0, allHeight);
...
//        [g_xmpp sendMessage:msg roomName:self.roomJid];//发送消息
        ...
        if (self.isGroupMessages) {
            ...
            [msg insert:self.roomJid];
            [g_xmpp sendMessage:msg roomName:self.roomJid];//群发送消息
        }
...
-(NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
    if (XL_Hidden_System_Message == 1) {
        for (JXMessageObject *msg in [_array reverseObjectEnumerator]) {
            if ([msg.type intValue] == kWCMessageTypeRemind && isHiddenMsg(msg.content)) {
                [_array removeObject:msg];
            }
        }
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch ([msg.type intValue]) {
        ...
        case kWCMessageTypeRedPacket: {
            ...
            CGFloat redPacketCellHeight = [JXRedPacketCell getChatCellHeight:msg];
            redPacketCellHeight += 10;
            redPacketCellHeight = redPacketCellHeight * 0.9;
            return redPacketCellHeight;
        }
            break;
        case kWCMessageTypeTransfer: {
            
            CGFloat transferCellHeight = [JXTransferCell getChatCellHeight:msg];
            transferCellHeight += 10;
            transferCellHeight = transferCellHeight * 0.9;
...
//                    self.title = [NSString stringWithFormat:@"%@(%@)",chatPerson.remarkName.length > 0 ? chatPerson.remarkName : chatPerson.userNickname,str];
    ...
    if (self.roomJid || msg.isGroup) {//是房间
        ...
        if ([msg.toUserId isEqualToString:chatPerson.userId]||[msg.toUserId isEqualToString:self.roomJid]) {//第一个判断时从MsgView进入，第二个从GroupView进入
            if (XL_Hidden_System_Message == 1 && [msg.type intValue] == kWCMessageTypeRemind && !isHiddenMsg(msg.content)) {
                [self showOneMsg:msg];
            }else{
                [self showOneMsg:msg];
            }
...
//    NSLog(@"newReceipt");
    ...
    if ([msg.type intValue] == kWCMessageTypeWithdraw) {
        ...
        [msg1 notifyNewMsg];
        [_wait stop];
        if ([msg1.content containsString:@"撤回了一条消息"] || [msg1.content containsString:@"撤回了一條消息"]) {
            [_table reloadData];
        }else{
...
//        return;
    ...
    _recordBtn.layer.borderColor = [HEXCOLOR(0x000000) CGColor];
...
- (void) setTableFooterFrame:(UITextView *) textView {
    ...
    textView.layer.cornerRadius = size.height*0.5;
...
-(void)actionQuit{
    ...
    if (g_mainVC.msgVc.array.count > 0) {
        [g_mainVC.msgVc.tableView reloadData];
...
-(void) didServerResultSucces:(JXConnection*)aDownload dict:(NSDictionary*)dict array:(NSArray*)array1{
    ...
    if ([aDownload.action isEqualToString:act_roomJoin]) {
        [g_server getRoom:self.scanString toView:self];
    }
...
//                self.roomUserId = [dict objectForKey:@"userId"];
    ...
    if( [aDownload.action isEqualToString:act_roomMemberGet] ){
        _disableSay = [[dict objectForKey:@"memberTalkTime"] longLongValue];
...
//        if ([dict[@"packet"][@"type"] intValue] != 3) {
        NSArray*userIds = dict[@"packet"][@"userIds"];
        NSString *userId = [NSString stringWithFormat:@"%@",[[dict objectForKey:@"packet"] objectForKey:@"userId"]];
       
        [_wait stop];
        if (self.roomJid.length > 0) {
            NSString *count = [NSString stringWithFormat:@"%@",[[dict objectForKey:@"packet"] objectForKey:@"count"]];
            if (userIds.count == [count integerValue]) {
                ...
                for (NSNumber *idInt in userIds) {
                    NSString *idStr = [NSString stringWithFormat:@"%@",idInt];
                     if ([idStr isEqualToString:MY_USER_ID]) {
                         JXredPacketDetailVC * redPacketDetailVC = [[JXredPacketDetailVC alloc]init];
                         redPacketDetailVC.dataDict = [[NSDictionary alloc]initWithDictionary:dict];
                         redPacketDetailVC.isGroup = self.room.roomId.length > 0;
                         [g_navigation pushViewController:redPacketDetailVC animated:YES];
...
//        NSLog(@"看看%@",dict);
        ...
        _openImgV.userInteractionEnabled = YES;
    ...
    if ([aDownload.action isEqualToString:act_roomGetRoom]) {
        ...
        if ([dict objectForKey:@"jid"]) {
            ...
            if (![dict objectForKey:@"member"]) { // 踢出群组时返回主页
...
//刷新禁言状态
-(void)updateTalkTime:(NSDictionary*)dict
{
    _disableSay = [[[dict objectForKey:@"member"]objectForKey:@"memberTalkTime"] longLongValue]; //单人禁言
...
//                    _shareMore.enabled = NO;
    ...
    if([[NSDate date] timeIntervalSince1970] <= _disableSay && !_isAdmin){
        ...
        [self hideKeyboard:NO];
        _talkTimeLabel.hidden = NO;
        _talkTimeLabel.text = [NSString stringWithFormat:@"%@%@",s,Localized(@"JXChatVC_GagTime")];
        _messageText.userInteractionEnabled = NO;
        _shareMore.enabled = YES;
        _recordBtnLeft.enabled = NO;
        _btnFace.enabled = NO;
...
//        [g_window addSubview:redPacketDetailVC.view];
    ...
    if ([aDownload.action isEqualToString:act_openRedPacket]) {
        ...
        if ([dict[@"resultMsg"]  isEqual: @"你的余额不足以赔付，请充值后领取！"]) {
            
            // 提示
            [g_App showAlert:@"你的余额不足以赔付，请充值后领取！"];
        }
        else {
...
//            [self refresh:nil];
    ...
    //判断创建是当前用户是否是群主
    if ([p.userId isEqualToString: g_myself.userId]) {
...
//    [str appendString:[NSString stringWithFormat:@"%ld",(long)[msg.timeSend timeIntervalSince1970]]];
...
//    [self fetchRedPacket:msg.objectId];
    [g_server getRedPacket:msg.objectId toView:self];
...
- (void)openReadPacket {
    _openImgV.userInteractionEnabled = NO;
    ...
    _openImgV.animationDuration = 0.15f;
    ...
    if (range.location != NSNotFound) {
        ...
        if ([stringValue rangeOfString:@"=user"].location != NSNotFound) {
           ...
           [g_server userGetByAccountWithAccount:idStr toView:self];
            ...
            NSArray *tempArray = [stringValue componentsSeparatedByString:@"shikuId="];
            self.scanString = tempArray.lastObject;
            [g_server roomJoinRoom:tempArray.lastObject toView:self];
//            JXRoomMemberVC* vc = [JXRoomMemberVC alloc];
//            vc.roomId = idStr;
//            vc = [vc init];
//            [g_navigation pushViewController:vc animated:YES];
...
#pragma mark----刷新禁言配置
- (void)onUpdateMuteRemind:(NSNotification *)notif {
//    NSLog(@"刷新禁言配置");
    if (self.roomJid) {
        [g_server roomGetRoom:self.roomId toView:self]; // 刷新群组设置
    }
}
// 回复消息点击
- (void)onDidReply:(NSNotification *)notif {
    ...
    for (JXMessageObject *msg1 in _array) {
        if ([msgObj.messageId isEqualToString:msg1.messageId]) {
            ...
            [_table scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0] atScrollPosition:UITableViewScrollPositionTop animated:NO];
        }
    }
