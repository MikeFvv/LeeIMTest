...
//    UILabel *titleL = [[UILabel alloc]initWithFrame:CGRectMake(16, statusHeight, JX_SCREEN_WIDTH*0.5, 44)];
    ...
    titleL.text = Localized(@"JX_MailList");
    titleL.font = [UIFont boldSystemFontOfSize:18];
    [self.tableHeader addSubview: titleL];
    
    /// ooo<<< 这里添加好友功能 后台开启关闭
    if ([g_App.config.hideSearchByFriends intValue] == 1) {
        ...
        NSString *image = @"message_add_friend_black";
...
//    [self.view addSubview:_topSiftView];
    ...
    //搜索输入框
    // 减少了4个item
//    backView = [[UIView alloc] initWithFrame:CGRectMake(0, JX_SCREEN_TOP, JX_SCREEN_WIDTH, 503)];
    backView = [[UIView alloc] initWithFrame:CGRectMake(0, JX_SCREEN_TOP, JX_SCREEN_WIDTH, 503-HEIGHT*4)];
...
//
//    [self createButton:nil superView:_menuView];
  ...
  /*
    ...
    self.friendNewMsgNum = [[UILabel alloc] initWithFrame:CGRectMake(button.frame.size.width-leftInset - 10, 20-5, 25, 25)];
...
- (void)showMenuView { // 显示菜单栏
    ...
    backFrame.size.height = _btnHeight + 56;
...
-(void)onMore:(UIButton *)sender{
    JXFriendAddViewController *vc = [[JXFriendAddViewController alloc] init];
    ...
    NSMutableArray *titles = [NSMutableArray arrayWithArray:@[Localized(@"JX_AddFriends"),Localized(@"JX_LaunchGroupChat"),  Localized(@"JX_Receiving"),Localized(@"JX_SearchPublicNumber")]];
     NSMutableArray *images = [NSMutableArray arrayWithArray:@[@"message_add_friend_black",@"message_creat_group_black",  @"message_near_receiving",@"message_search_publicNumber"]];
...
//    header.textLabel.textColor = [UIColor redColor];
    header.contentView.backgroundColor = HEXCOLOR(0xF4F4F4);
...
-(void)createButton:(SEL)click superView:(UIView *)superView{
    _menuView = [[UIView alloc] initWithFrame:CGRectMake(0, 40, JX_SCREEN_WIDTH, superView.frame.size.height-40)];
    _btnHeight = _menuView.frame.size.height;
    [superView addSubview:_menuView];
...
//    NSArray *titleArray = @[Localized(@"JXNewFriendVC_NewFirend"),Localized(@"OrganizVC_Organiz"),Localized(@"JX_MobilePhoneContacts"), Localized(@"JX_PublicNumber"),Localized(@"JX_ManyPerChat"),Localized(@"JX_BlackList"),Localized(@"JX_MyDevices"),Localized(@"JX_Label")];
//
...
//    NSArray *titleArray = @[Localized(@"JXNewFriendVC_NewFirend"),@"",Localized(@"JX_MobilePhoneContacts"), @"",Localized(@"JX_ManyPerChat"),@"",@"",Localized(@"JX_Label")];
    ...
    for (int i=0; i<titleArray.count; i++) {
        ...
        UIImageView * icon = [[UIImageView alloc]initWithFrame:CGRectMake(14, (HEIGHT-42)*0.5, 42, 42)];
        icon.contentMode = UIViewContentModeScaleAspectFill;
        icon.image = [UIImage imageNamed:iconArray[i]];
        [btnView addSubview:icon];
        // lewis
//        UILabel * titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(icon.right+8, 9, JX_SCREEN_WIDTH-icon.right-15, 16)];
        UILabel * titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(icon.right+8, 20, JX_SCREEN_WIDTH-icon.right-15, 16)];
...
//        [superView addSubview:btnView];
        [_menuView addSubview:btnView];
        // lewis 不显示subtitle
//        UILabel * subTitleLabel = [[UILabel alloc]initWithFrame:CGRectMake(icon.right+8, titleLabel.bottom+8, JX_SCREEN_WIDTH-icon.right-15, 14)];
...
//        [btnView addSubview:subTitleLabel];
        ...
        switch (btnView.tag) {
            ...
            case 1:
            {
                ...
                UITapGestureRecognizer * tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(myColleaguesAction:)];
                [btnView addGestureRecognizer:tap];
            }
                break;
            case 2:
            ...
            case 5:
            {
                ...
                UITapGestureRecognizer * tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(blackFriendAction:)];
                [btnView addGestureRecognizer:tap];
            }
                break;
            case 6:
