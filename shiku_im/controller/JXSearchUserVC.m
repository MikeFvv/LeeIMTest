...
//        iv = [self createButton:[NSString stringWithFormat:@"%@%@",name,phoneN] drawTop:NO drawBottom:YES must:NO click:nil];
         iv = [self createButton:@"请输入用户名" drawTop:NO drawBottom:YES must:NO click:nil];
...
//        _name = [self createTextField:iv default:job.name hint:[NSString stringWithFormat:@"%@%@",input,phoneN]];
         _name = [self createTextField:iv default:job.name hint:@"请输入用户名"];
        ...
        */
        ...
        [_btn setTitle:Localized(@"JX_Seach") forState:0];
        [_btn setBackgroundColor:THEMECOLOR];
        [_btn addTarget:self action:@selector(onSearch) forControlEvents:UIControlEventTouchUpInside];
        _btn.custom_acceptEventInterval = .25f;
        _btn.frame = CGRectMake(20, h, JX_SCREEN_WIDTH-40, 44);
