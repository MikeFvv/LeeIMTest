...
- (void)createViews {
    ...
    _head.layer.cornerRadius = 3.0f;// lewis _head.frame.size.width/2;
...
//    iv = [self createButton:Localized(@"JXQR_QRImage") drawTop:YES drawBottom:YES must:NO click:@selector(showUserQRCode) superView:_baseView];
//    iv.frame = CGRectMake(0, h, JX_SCREEN_WIDTH, HEIGHT);
...
//
//    // 显示手机号
//    iv = [self createButton:Localized(@"JX_MobilePhoneNo.") drawTop:NO drawBottom:YES must:NO click:nil superView:_baseView];
//    iv.frame = CGRectMake(0, h, JX_SCREEN_WIDTH, HEIGHT);
//    _showNImgV = iv;
//    _showNum = [self createLabel:iv default:self.user.telephone];
//    h+=iv.frame.size.height;
...
#pragma mark 消息免打扰
    ...
    if (!self.isJustShow) {
        ...
        if([self.userId intValue] != [MY_USER_ID intValue]){
            ...
            _btn.frame = CGRectMake(20, h+30, JX_SCREEN_WIDTH-40, 44);
...
//    [btn release];
    ...
    if(must){
        UILabel* p = [[UILabel alloc] initWithFrame:CGRectMake(INSETS, (HEIGHT - 30) / 2, 20, 30)];
        p.text = @"*";
...
//        [p release];
    ...
    JXLabel* p = [[JXLabel alloc] initWithFrame:CGRectMake(20, (HEIGHT - 30) / 2, 200, 30)];
    p.text = title;
