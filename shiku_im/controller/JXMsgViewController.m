...
- (id)init
{
    ...
    if (self) {
        ...
        _table.backgroundColor = RGB(240, 240, 240);
...
//    UILabel *msgStatusL = [[UILabel alloc]initWithFrame:CGRectMake(16, statusHeight, JX_SCREEN_WIDTH*0.5, 44)];
    UILabel *msgStatusL = [[UILabel alloc]initWithFrame:CGRectMake(JX_SCREEN_WIDTH/4, statusHeight, JX_SCREEN_WIDTH*0.5, 44)];
    msgStatusL.textAlignment = NSTextAlignmentCenter;
    NSMutableAttributedString* attri = [[NSMutableAttributedString alloc]initWithString:Localized(@"JXMainViewController_Message")];
    msgStatusL.attributedText = attri;
    msgStatusL.font = [UIFont boldSystemFontOfSize:18];
...
//    [self.tableHeader addSubview:self.editBtn];
    ...
    UIView *backView = [[UIView alloc] initWithFrame:CGRectMake(0, JX_SCREEN_TOP, JX_SCREEN_WIDTH, 65)];
    backView.backgroundColor = [UIColor whiteColor];
    [self.view addSubview:backView];

    _seekTextField = [[UITextField alloc] initWithFrame:CGRectMake(15, 10, backView.frame.size.width - 30, 40)];
    _seekTextField.placeholder = Localized(@"JX_Seach");
    _seekTextField.backgroundColor = RGB(244, 244, 244);
    _seekTextField.textColor = [UIColor blackColor];
    [_seekTextField setFont:SYSFONT(16)];
...
//    imageView.center = CGPointMake(leftView.frame.size.width/2, leftView.frame.size.height/2);
    ...
    _seekTextField.layer.cornerRadius = _seekTextField.height*0.5;
...
//    NSMutableArray *titles = [NSMutableArray arrayWithArray:@[Localized(@"JX_LaunchGroupChat"), Localized(@"JX_AddFriends"), Localized(@"JX_Scan"), Localized(@"JXNearVC_NearPer"),Localized(@"JX_Receiving"),Localized(@"JX_SearchPublicNumber")]];
//    NSMutableArray *images = [NSMutableArray arrayWithArray:@[@"message_creat_group_black", @"message_add_friend_black", @"messaeg_scnning_black", @"message_near_person_black",@"message_near_receiving",@"message_yijianqunfa_blcack"]];
//    NSMutableArray *sels = [NSMutableArray arrayWithArray:@[@"onNewRoom", @"onSearch", @"showScanViewController", @"onNear",@"onReceiving",@"searchPublicNumber"]];
    
//    NSMutableArray *titles = [NSMutableArray arrayWithArray:@[Localized(@"JX_AddFriends"),Localized(@"JX_LaunchGroupChat"),  Localized(@"JX_Receiving"),Localized(@"JX_SearchPublicNumber")]];
//    NSMutableArray *images = [NSMutableArray arrayWithArray:@[@"message_add_friend_black",@"message_creat_group_black",  @"message_near_receiving",@"message_yijianqunfa_blcack"]];
//    NSMutableArray *sels = [NSMutableArray arrayWithArray:@[@"onSearch",@"onNewRoom",  @"onReceiving",@"searchPublicNumber"]];
    
    // lewis
    NSMutableArray *titles = [NSMutableArray arrayWithArray:@[Localized(@"JX_LaunchGroupChat"),Localized(@"JX_AddFriends"), Localized(@"JX_Scan"),Localized(@"JXGroupMessages"),Localized(@"JX_Receiving")]];
    NSMutableArray *images = [NSMutableArray arrayWithArray:@[@"message_creat_group_black",@"message_add_friend_black",@"messaeg_scnning_black",  @"message_yijianqunfa_blcack",@"message_near_receiving"]];
    NSMutableArray *sels = [NSMutableArray arrayWithArray:@[@"onNewRoom",@"onSearch",@"showScanViewController",  @"groupMessages",@"onReceiving"]];
...
//    [dict.message.type intValue] == kRoomRemind_RoomAllBanned
    if (XL_Hidden_System_Message == 1) {
        if ([dict.message.type intValue] == kWCMessageTypeRemind && isHiddenMsg(dict.message.content)) {
            lastContet = @"";
        }
    }
...
//        cell.selectionStyle = UITableViewCellSelectionStyleGray;
    ...
    if (dict.user.topTime) {
        cell.backgroundColor = HEXCOLOR(0xF0F1F2);
    }
    else {
...
//        [_array release];
    ...
    for (NSInteger i = 0; i < p.count; i ++) {
        ...
        if ([obj.user.userId isEqualToString: CALL_CENTER_USERID]) { //  系统消息
            obj.user.userNickname = [NSString stringWithFormat:@"%@客服",XL_APP_NAME];
        } if ([obj.user.userId isEqualToString: SHIKU_TRANSFER]) {
            obj.user.userNickname = @"支付助手";
        }
    }
...
-(void)onLoginChanged:(NSNotification *)notifacation{
    ...
    switch ([JXXMPP sharedInstance].isLogined){
        case login_status_ing:{
            titleStr = Localized(@"JXMsgViewController_GoingOff");
            imgStr = @"online_lianjiezhong";
//            self.title = Localized(@"JXMsgViewController_GoingOff");
        ...
        case login_status_no:{
            titleStr = Localized(@"JXMsgViewController_OffLine");
            imgStr = @"online_lixian";
...
//            }
        ...
        case login_status_yes:{
            titleStr = Localized(@"JXMsgViewController_OnLine");
            imgStr = @"online_zaixian";
...
-(void)getTotalNewMsgCount{
    ...
    for (JXMsgAndUserObject * dict in _array) {
        ...
        if (![dict.user.userId isEqualToString:FRIEND_CENTER_USERID]) {
            ...
            //判断是否是免打扰
            if (![dict.user.offlineNoPushMsg boolValue]) {
...
//            }
    ...
    if([p.type intValue] == kRoomRemind_AddMember){
        ...
        int n = [[NSDate date] timeIntervalSince1970]-[p.time timeIntervalSince1970];
         if(n>10)//如果时间差超过30秒，则放弃
             return;
