...
WavWriter::WavWriter(const char *filename, int sampleRate, int bitsPerSample, int channels) 
{
	...
	NSString *filePath        = [documentPath stringByAppendingPathComponent:[NSString stringWithFormat:@"%s", filename]];
	NSLog(@"documentPath=%@", documentPath);
	
	wav = fopen([filePath cStringUsingEncoding:NSASCIIStringEncoding], "wb");
	if (wav == NULL)
		return;
	dataLength = 0;
