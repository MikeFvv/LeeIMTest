...
#define ACTIONSHEET_BACKGROUNDCOLOR             [UIColor colorWithRed:83/255.00f green:202/255.00f blue:195/255.00f alpha:1]
...
#define BUTTON_WIDTH                            (JX_SCREEN_WIDTH - 60)
...
//            if (button.tag == self.postionIndexNumber-1) {
    ...
    if (self.delegate) {
        if ([self.delegate respondsToSelector:@selector(didClickOnButtonIndex:buttonIndex:)] == YES) {
            [self.delegate didClickOnButtonIndex:self buttonIndex:(int)button.tag];
        }
    }
