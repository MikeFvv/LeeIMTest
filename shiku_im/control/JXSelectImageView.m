...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        if (([g_App.isShowGroupRedPacket intValue] == 1 && !self.isGroupMessages && !self.isDevice) || (!self.isGroup && [g_App.isShowRedPacket intValue] == 1)) {
            // 发红包
            n = (n + 1) >= 4 ? 0 : n + 1;
            m += 1;
            X = m >8 ? SELECTIMAGE_WIDTH *n + (n+1)*inset+JX_SCREEN_WIDTH : SELECTIMAGE_WIDTH *n + (n+1)*inset;
                              ...
                              title:(NSString*)title
{
    ...
    btn.frame = CGRectMake(5, 0, 40, 40);
    [v addSubview:btn];
