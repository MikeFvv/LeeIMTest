...
//        captionSize = CGSizeMake(captionSize.width + 30, captionSize.height + 30);
...
//						captionSize = [caption sizeWithFont:[UIFont boldSystemFontOfSize:14] constrainedToSize:CGSizeMake(progressRect.size.width, 200) lineBreakMode:UILineBreakModeWordWrap];
					...
					targetBounds = CGRectMake(-100, 0, captionSize.width + p.margin * 2, captionSize.height+p.margin*2+adjustment);
				}
...
//					captionSize = [caption sizeWithFont:[UIFont boldSystemFontOfSize:14] constrainedToSize:CGSizeMake(160, 200) lineBreakMode:UILineBreakModeWordWrap];
				...
				if (marginY < p.margin) {
					targetBounds = CGRectMake(-p.margin, 0, targetBounds.size.width, captionSize.height + 2 * p.margin+adjustment);
					marginY = p.margin;
				}
			} else if (p.accessoryPosition == ATMHudAccessoryPositionLeft || p.accessoryPosition == ATMHudAccessoryPositionRight) {
				if (image) {
	...
	switch (mode) {
		case ATMHudApplyModeShow: {
			if (CGPointEqualToPoint(p.center, CGPointZero)) {
				self.frame = CGRectMake((self.superview.bounds.size.width - targetBounds.size.width) * 0.5 - p.margin, (self.superview.bounds.size.height-targetBounds.size.height)*0.5, targetBounds.size.width, targetBounds.size.height);
			} else {
