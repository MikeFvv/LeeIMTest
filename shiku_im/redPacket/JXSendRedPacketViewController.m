...
//    g_navigation.navigationView.backgroundColor = [UIColor redColor];
    g_navigation.imageView.image = [UIImage createImageWithColor:[UIColor redColor]];
    g_navigation.tableHeader.backgroundColor = [UIColor greenColor];
    self.tableHeader.backgroundColor = [UIColor blueColor];
    self.imageView.image = [UIImage createImageWithColor:HEXCOLOR(0xD85940)];
}
...
//    textField.text = str;
    ...
    if (textField == inputView.moneyTextField) {
        // 对输入的金额进行回显
        inputView.inputMoneyShowLabel.text = [NSString stringWithFormat:@"￥%@", str];
        return YES;
    }
