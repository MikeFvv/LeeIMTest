...
//            }else {
            versionManage *config = g_config;
            NSLog(@"%@",config.apiUrl);
            NSLog(@"%@",g_config.apiUrl);
                s = g_config.apiUrl;
//            }
...
-(void) doError:(JXConnection*)task dict:(NSDictionary*)dict resultMsg:(NSString*)string errorMsg:(NSString*)errorMsg{
    ...
    if(n != hide_error){
        //        if(task.showError)
        if (![task.action isEqualToString:@"upload/UploadAvatarServlet"]) {
...
-(void)getHeadImageLarge:(NSString*)userId imageView:(UIImageView*)iv{
...
//        iv.image = [UIImage imageNamed:[NSString stringWithFormat:@"im_10000"]];
    ...
    [iv sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:@"avatar_normal"] options:SDWebImageRefreshCached];
}
...
//        return;
    ...
    [iv sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:placeholderImage options:SDWebImageRefreshCached];
//    [self getDefultHeadImage:[NSURL URLWithString:url] userId:userId userName:userName placeholderImage:placeholderImage iv:iv];
}
...
-(void)doSaveUser:(NSDictionary*)dict{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:myself.userId forKey:@"userId"];
...
//    [p setPostValue:[NSNumber numberWithInt:2] forKey:@"uploadFlag"];
    ...
    if([file hasSuffix:@"jpg"]||[file hasSuffix:@"png"]||[file hasSuffix:@"jpeg"]){
        //是图片
        NSData *filedata = UIImageJPEGRepresentation([UIImage imageWithContentsOfFile:file], 0.5);
        [p setData:filedata forKey:[file lastPathComponent] messageId:nil];
    }else{
        //其他类型
        [p setData:[NSData dataWithContentsOfFile:file] forKey:[file lastPathComponent] messageId:nil];
    }
...
-(void)resetPayPwd:(NSString*)telephone areaCode:(NSString *)areaCode randcode:(NSString*)randcode newPwd:(NSString*)newPassword toView:(id)toView{

    if(telephone==nil || newPassword==nil || randcode==nil)
        return;

    JXConnection* p = [self addTask:act_PayPwdReset param:nil toView:toView];
    if (g_server.isLogin) {
         [p setPostValue:access_token forKey:@"access_token"];
     }
    [p setPostValue:telephone forKey:@"telephone"];
...
-(void)getSetting:(id)toView{
    ...
    if ([g_myself.telephone isEqualToString:@"13807380000"]) {
        urlStr = act_LoginConfig;
    }else{
        urlStr = act_Config;
    }
    JXConnection* p = [self addTask:urlStr param:nil toView:toView];
    NSString *area = [g_default objectForKey:kLocationArea];
...
-(void)setDisableSay:(NSString*)roomId member:(memberData*)member  toView:(id)toView{
    JXConnection* p = [self addTask:act_roomMemberSet param:nil toView:toView];
    [p setPostValue:roomId forKey:@"roomId"];
    [p setPostValue:[NSNumber numberWithLongLong:member.talkTime] forKey:@"memberTalkTime"];
    [p setPostValue:[NSNumber numberWithLong:member.userId] forKey:@"userId"];
    [p setPostValue:access_token forKey:@"access_token"];
    [p go];
}
...
// 加入群组
- (void)roomJoinRoom:(NSString *)roomId toView:(id)toView {
    JXConnection* p = [self addTask:act_roomJoin param:nil toView:toView];
    [p setPostValue:self.access_token forKey:@"access_token"];
    [p setPostValue:roomId forKey:@"roomId"];
    [p setPostValue:@"2" forKey:@"type"];
    [p go];
}
// 朋友圈纯视频接口
...
// 绑定银行卡
- (void)bindBankCardWithBankBrandId:(NSString *)bankBrandId secret:(NSString *)secret  cardNo:(NSString *)cradNo userName:(NSString *)userName openCradAddress:(NSString *)openCardAddress toView:(id)toView{
    
    JXConnection* p = [self addTask:act_BindBankCard param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
    [p setPostValue:openCardAddress forKey:@"openBankAddr"];
    [p setPostValue:secret forKey:@"secret"];
    [p setPostValue:bankBrandId forKey:@"bankBrandId"];
    [p setPostValue:cradNo forKey:@"cardNo"];
...
// 设置实名状态
- (void)UpdateFourElementsToView:(id)toView{
    
    JXConnection* p = [self addTask:setUpdateFourElements param:nil toView:toView];
    [p setPostValue:MY_USER_ID forKey:@"userId"];

    [p go];
}
// 获取实名状态
- (void)GetUpdateFourElementsStateToView:(id)toView{
    
    JXConnection* p = [self addTask:getUpdateFourElements param:nil toView:toView];
    [p setPostValue:MY_USER_ID forKey:@"userId"];

    [p go];
}
- (void)getBindBankCardListWithSecret:(NSString *)secret toView:(id)toView {
    
    JXConnection* p = [self addTask:act_GetBankCardList param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
    [p setPostValue:secret forKey:@"secret"];
...
- (void)theMessaging:(NSString *)content fromUserID:(NSString *)fromUserId jid:(NSString *)jid toView:(id)toView{
    
    JXConnection* p = [self addTask:act_GetNiuniuSendXiazhu param:nil toView:toView];
//    [p setPostValue:access_token forKey:@"access_token"];
//    [p setPostValue:secret forKey:@"secret"];
    [p setPostValue:content forKey:@"content"];
    [p setPostValue:fromUserId forKey:@"fromUserId"];
    [p setPostValue:jid forKey:@"jid"];
    [p go];
...
- (void)applyWithDraw:(NSString *)secret cardId:(NSString *)cardId money:(NSString *)money toView:(id)toView{
    
    JXConnection* p = [self addTask:act_WithDraw param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
    [p setPostValue:secret forKey:@"secret"];
    [p setPostValue:cardId forKey:@"cardId"];
    [p setPostValue:money forKey:@"money"];
...
- (void)deleteBindCard:(NSString *)secret cardId:(NSString *)cardId toView:(id)toView{
    
    JXConnection* p = [self addTask:act_DeleteBindBankCard param:nil toView:toView];
       [p setPostValue:access_token forKey:@"access_token"];
       [p setPostValue:secret forKey:@"secret"];
       [p setPostValue:cardId forKey:@"cardId"];
...
- (void)getThirdPayWithPayType:(NSString *)payType money:(NSString *)money toView:(id)toView{
    
    JXConnection* p = [self addTask:act_GetThirdPayAddress param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
    [p setPostValue:money forKey:@"money"];
...
- (void)getAgoraInfo:(id)toView fileName:(NSString *)fileName {
    ...
    JXConnection* p = [self addTask:act_GetAgoraInfo param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
...
- (void)getXLPayMoney:(NSString *)money UserId:(NSString *)userid secret:(NSString *)secret time:(NSNumber *)time toView:(id)toView{
    
    JXConnection* p = [self addTask:@"unionpay/consume" param:nil toView:toView];
    [p setPostValue:access_token forKey:@"access_token"];
    [p setPostValue:money forKey:@"amount"];
    [p setPostValue:userid forKey:@"userid"];
//    [p setPostValue:secret forKey:@"secret"];
//    [p setPostValue:time forKey:@"time"];
    
    [p setPostValue:@"" forKey:@"secret"];
    
    long time1 = (long)[[NSDate date] timeIntervalSince1970];
    [p setPostValue:@(time1) forKey:@"time"];
    [p go];
}
@end
