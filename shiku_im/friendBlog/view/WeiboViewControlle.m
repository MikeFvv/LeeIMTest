...
@interface WeiboViewControlle ()<UIAlertViewDelegate,JXActionSheetVCDelegate,JXSelectMenuViewDelegate,JXMenuViewDelegate,WeiboCellDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,JXCameraVCDelegate>
{
    ...
    UIImageView *_titleBkgImageView;
    UIButton* _rightBtn;
}
...
//        _input.delegate = self;
        ...
        self.heightHeader = -statusHeight; // JX_SCREEN_TOP;
#ifdef IS_SHOW_MENU
        self.isGotoBack = YES;
#endif
        self.isGotoBack = YES;
        ...
        [self.gotoBackBtn setImage:[UIImage imageNamed:@"title_back_white"] forState:UIControlStateNormal];
        _titleBkgImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self_width, self.heightHeader)];
        if (THESIMPLESTYLE) {
            _titleBkgImageView.image = [[UIImage imageNamed:@"navBarBackground"] imageWithTintColor:[UIColor whiteColor]];
        }else {
            _titleBkgImageView.image = [g_theme themeTintImage:@"navBarBackground"];//[UIImage imageNamed:@"navBarBackground"];
        }
        _titleBkgImageView.hidden = YES;
        [self.view addSubview:_titleBkgImageView];
        [self.view bringSubviewToFront:self.tableHeader];
        
        self.heightHeader = -statusHeight;
        
        // lewis
//        _table.frame =CGRectMake(0,0,self_width,self_height-JX_SCREEN_BOTTOM);
        self.tableHeader.backgroundColor = [UIColor clearColor];
        
        [[[self.tableHeader subviews] objectAtIndex:0] removeFromSuperview];
        self.tableHeader.frame = CGRectMake(0, 0, self_width,JX_SCREEN_TOP);
...
//    NSInteger statusHeight = [UIApplication sharedApplication].statusBarFrame.size.height;
//    self.heightHeader = -statusHeight;
    _table.frame =CGRectMake(0,self.heightHeader,self_width,self_height-self.heightHeader-self.heightFooter);//JX_SCREEN_BOTTOM);
    [self stopLoading];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary*)change context:(void *)context {
    if (object == self.tableView) {
        ...
        if (point.y < 60) {
            ...
            _titleBkgImageView.hidden = YES;
            [self.gotoBackBtn setImage:[UIImage imageNamed:@"title_back_white"] forState:UIControlStateNormal];
            [_rightBtn setImage:[UIImage imageNamed:THESIMPLESTYLE ? @"xiangji-2" : @"xiangji"] forState:UIControlStateNormal];
        } else if (point.y < 400) {
            self.title = Localized(@"JXUserInfoVC_Space");
            _titleBkgImageView.hidden = NO;
            _titleBkgImageView.alpha = point.y/400;
            [self.gotoBackBtn setImage:[UIImage imageNamed:@"title_back_black"] forState:UIControlStateNormal];
            [_rightBtn setImage:[UIImage imageNamed:THESIMPLESTYLE ? @"xiangji" : @"xiangji-2"] forState:UIControlStateNormal];
        } else {
            _titleBkgImageView.alpha = 1;
        }
...
//    NSString *image = THESIMPLESTYLE ? @"im_003_more_button_black" : @"im_003_more_button_normal";
    NSString *image = THESIMPLESTYLE ? @"xiangji-2" : @"xiangji";
    _rightBtn = [UIFactory createButtonWithImage:image
                                           highlight:nil
                                              target:self
                                            selector:@selector(onAddMsg:)];
    _rightBtn.frame = CGRectMake(JX_SCREEN_WIDTH - 40, JX_SCREEN_TOP - 34, 24, 24);
    [self.tableHeader addSubview:_rightBtn];
}
#pragma mark   ---------------发说说------------
...
//        [head addSubview:btn];
    ...
    v.layer.cornerRadius = 3.0f;// lewis40.f;
    ...
    iv.layer.cornerRadius = 3.0f;// lewis 37.f;
    iv.delegate = self;
