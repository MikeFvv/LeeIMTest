...
//    config.XMPPDomain = @"http://imapi.suliao6688.com";
    config.backUrl = XL_API_BACK_URL;
    config.apiUrl = XL_API_API_URL;
    config.uploadUrl = XL_API_UPLOAD_URL;
    config.downloadAvatarUrl = XL_API_DOWNLOAD_AVATAR_URL;
    config.downloadUrl = XL_API_DOWNLOAD_URL;
    config.XMPPHost = XL_API_XMPP_HOST;
    config.XMPPDomain = XL_API_XMPP_DOMAIN;
    config.XMPPPingTime = 10;
    config.XMPPTimeout = 300;
...
//    [NSThread sleepForTimeInterval:0.3];
    // 设置主题颜色
    [[SkinManage sharedInstance] switchSkinIndex:SkinType_simple_Blue];
    [self showLoginUI];
...
//    }
//
//    ret = [_baiduMapManager start:@"WGLPIpcbY1T0bzitLq1zTKsFb8n9QdB4"  generalDelegate:nil];
//
//    if (!ret)
...
//    NSString *advertisingId = [[[ASIdentifierManager sharedManager] advertisingIdentifier] UUIDString];
    ...
    [JPUSHService setupWithOption:launchOptions appKey:@"7706dd8089d035b1c25c2981"
...
-(void)imageCacheRef{
    ...
    imgDownloader.headersFilter = ^SDHTTPHeadersDictionary * _Nullable(NSURL * _Nullable url, SDHTTPHeadersDictionary * _Nullable headers) {
...
- (void)setUserAgent {
    ...
    [webView evaluateJavaScript:@"navigator.userAgent" completionHandler:^(id obj, NSError * _Nullable error) {

        if([obj isKindOfClass:[NSString class]]) {
           NSString *newUA = [NSString stringWithFormat:@"%@ %@",obj,@"app-zhiliaoapp"];
...
//    if(mainVc==nil){
    ...
    g_navigation.rootViewController = nav;
//    }
...
//#endif
    ...
    if ([url.host isEqualToString:@"safepay"]) {
        ...
        return [[JXShareManage sharedManager] handleOpenURL:url delegate:nil];
    }
}
...
- (void)jpushNotificationCenter:(UNUserNotificationCenter *)center didReceiveNotificationResponse:(UNNotificationResponse *)response withCompletionHandler:(void (^)())completionHandler {
    ...
    if ([type isEqualToString:@"1"]) {
       ...
       array  = [[JXUserObject sharedInstance]fetchAllFriendsFromLocal];

        
    }else if ([type isEqualToString:@"2"]) {
        array  = [[JXUserObject sharedInstance]fetchAllRoomsFromLocal];
    }
     if ([type isEqualToString:@"1"] || [type isEqualToString:@"2"]) {
    for (JXUserObject *user  in array) {
        if ([user.userId isEqualToString:objectId]) {
            JXChatViewController *sendView = [[JXChatViewController alloc] init];
