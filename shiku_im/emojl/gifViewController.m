...
//                [iv release];
    ...
    for (int i = 0; i < faceArray.count; i++) {
        NSString * key = faceArray[i];
        if ([key isEqualToString:@"touzi.gif"]) {
            a = i;
        }
    }
    
    NSString * key = faceArray[a];
    SCGIFImageView* iv = imageArray[a];
    [faceArray removeObjectAtIndex:a];
    [imageArray removeObjectAtIndex:a];
    [faceArray insertObject:key atIndex:2];
    [imageArray insertObject:iv atIndex:2];
...
//    [_sv release];
    ...
    for(int i=0;i<maxPage;i++){
        ...
        for(int j=0;j<tempN * 2;j++){
            ...
            if (n == 2) {
                button.frame = CGRectMake(x+10, y+20, 40, 40);
            }
            button.tag = n;
...
-(void)actionSelect:(UIView*)sender
{
    ...
    if ([text isEqualToString:@"touzi.gif"]) {
        int x = arc4random() % 6;
        text = [NSString stringWithFormat:@"touzi_%d.gif",x+1];
    }
