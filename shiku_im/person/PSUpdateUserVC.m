...
//    h = INSETS*2+IMGSIZE;
    ...
    h = INSETS;
    iv = [self createButton:Localized(@"JX_ProfilePhoto") drawTop:NO drawBottom:NO must:NO click:@selector(pickImage)];
    iv.frame = CGRectMake(0, h, JX_SCREEN_WIDTH, HEIGHT*1.5);
    
    for (UIView *view in [iv subviews]) {
        CGFloat y = (iv.frame.size.height - view.frame.size.height)/2;
        view.frame = CGRectMake(view.frame.origin.x, y, view.frame.size.width, view.frame.size.height);
    }
    
    _head = [[JXImageView alloc] init];
    _head.frame = CGRectMake(JX_SCREEN_WIDTH-INSETS-3-HEIGHT*1.5, 10, HEIGHT*1.5-20, HEIGHT*1.5-20);
//    _head.image = [UIImage imageNamed:@"qrcodeImage"];
    _head.layer.cornerRadius = 3.0f;
    _head.layer.masksToBounds = YES;
    _head.layer.borderWidth = .5f;
    _head.layer.borderColor = [UIColor whiteColor].CGColor;
    s = user.userId;
    [g_server getHeadImageLarge:s userName:user.userNickname imageView:_head];
    [iv addSubview:_head];
    ...
    iv = [self createButton:Localized(@"JX_Name") drawTop:YES drawBottom:YES must:NO click:nil];
    ...
    // lewis
    _sex.hidden = YES;
    _sexLable = [self createLabel:iv default:city];
    _sexLable.text = [user.sex boolValue]?Localized(@"JX_Man"):Localized(@"JX_Wuman");
    
    [iv addSubview:_sex];
    //        [_sex release];
    h+=iv.frame.size.height;
    
    iv = [self createButton:Localized(@"JX_BirthDay") drawTop:NO drawBottom:YES must:NO click:nil];
    iv.frame = CGRectMake(0, h, JX_SCREEN_WIDTH, HEIGHT);
...
//        iv = [self createButton:Localized(@"JXUserInfoVC_Address") drawTop:NO drawBottom:NO must:NO click:@selector(onCity)];
        iv = [self createButton:Localized(@"JX_MyCity") drawTop:NO drawBottom:NO must:NO click:@selector(onCity)];
        iv.frame = CGRectMake(0, h, JX_SCREEN_WIDTH, HEIGHT);
    ...
    if (!self.isRegister) {
        iv = [self createButton:Localized(@"JX_MyQRImage") drawTop:NO drawBottom:NO must:NO click:@selector(showUserQRCode)];
        iv.frame = CGRectMake(0, h, JX_SCREEN_WIDTH, HEIGHT);
        ...
        if ([self.user.setAccountCount integerValue] > 0) {
            ...
            iv = [self createButton:@"Vk号" drawTop:NO drawBottom:YES must:NO click:nil];
        }else {
            
            iv = [self createButton:@"Vk号" drawTop:NO drawBottom:YES must:NO click:@selector(onShikuNum)];
        }
...
//        _btn = [UIFactory createCommonButton:Localized(@"JX_Update") target:self action:@selector(onUpdate)];
        _btn = [UIFactory createCommonButton:Localized(@"JX_Submit") target:self action:@selector(onUpdate)];
    _btn.layer.cornerRadius = 22;
    _btn.clipsToBounds = YES;
    _btn.custom_acceptEventInterval = 1.0f;
    _btn.frame = CGRectMake(20, h+20, JX_SCREEN_WIDTH-40, 44);
...
//    JXLabel* p = [[JXLabel alloc] initWithFrame:CGRectMake(30, 0, 130, HEIGHT)];
    JXLabel* p = [[JXLabel alloc] initWithFrame:CGRectMake(INSETS, 0, 130, HEIGHT)];
    p.text = title;
...
// lewis
-(void)onSex{
    JXActionSheetVC *actionVC = [[JXActionSheetVC alloc] initWithImages:@[] names:@[Localized(@"JX_Wuman"),Localized(@"JX_Man")]];
...
//    [_image retain];
    _head.image = _image;
    for (UILabel *label in _head.subviews) {
...
- (void)actionSheet:(JXActionSheetVC *)actionSheet didButtonWithIndex:(NSInteger)index {
    if (actionSheet.tag == 1) {
        ...
        if (index == 0) {
            ...
            _sexLable.text = Localized(@"JX_Wuman");
        } else {
            _sex.selectedSegmentIndex  = 1;
            _sexLable.text = Localized(@"JX_Man");
        }
    }
}
