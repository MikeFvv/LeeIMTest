...
//        [self.telAreaDelegate performSelector:self.didSelect withObject:[[_telAreaArray objectAtIndex:indexPath.row] objectForKey:@"prefix"]];
        NSDictionary *dict =  _telAreaArray[indexPath.row];
        NSString *area = [NSString stringWithFormat:@"%@,%@,%@",dict[@"country"],dict[@"region"],dict[@"city"]];
        [g_default setObject:area forKey:kLocationArea];
        
        NSLog(@"%@", _telAreaArray[indexPath.row]);
        [self.telAreaDelegate performSelectorOnMainThread:self.didSelect withObject:[_telAreaArray objectAtIndex:indexPath.row]  waitUntilDone:NO];
    }
    [self actionQuit];
}
