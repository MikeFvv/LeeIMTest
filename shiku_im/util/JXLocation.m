...
- (void)reverseGeocodeWithGoogleapi:(CLLocation *)location {
    ...
    [manager GET:urlString parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if ([[responseDic objectForKey:@"status"] isEqualToString:@"OK"]) {
            NSArray *returenArray = [responseDic objectForKey:@"results"];
            ...
            NSArray *arr = [addressDic objectForKey:@"address_components"];
            ...
            for (NSDictionary *dict in arr) {
                ...
                if ([type isEqualToString:@"country"]) {
                    ...
                    self.countryCode = [dict objectForKey:@"short_name"];
                    break;
                }
            }
            ...
            for (NSDictionary *dict in arr) {
                ...
                if ([type isEqualToString:@"locality"]) {
                    ...
                    self.cityName = [dict objectForKey:@"long_name"];
...
//                        self.cityId = [g_constant getTypeCityId:self.cityName];
            ...
            _address = [addressDic objectForKey:@"formatted_address"];
...
- (void)getLocationWithIp {
    ...
    [manager GET:urlString parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        // 国家代号
        self.countryCode = [responseDic objectForKey:@"country"];
        if (self.countryCode) {
            [[NSUserDefaults standardUserDefaults] setObject:self.countryCode forKey:kISOcountryCode];
            [[NSUserDefaults standardUserDefaults] synchronize];
        }
        ...
        self.cityName = [responseDic objectForKey:@"city"];
//        self.cityId = [g_constant getCityId:self.cityName Language:@"en"];
        
        // 如果城市id获取不到，就用省份查id（areas表里 有的外国省对应表里的城市）
        if (!self.cityId) {
            self.cityName = [responseDic objectForKey:@"region"];
//            self.cityId = [g_constant getCityId:self.cityName Language:@"en"];
        ...
        _address = [NSString stringWithFormat:@"%@%@",[responseDic objectForKey:@"region"],[responseDic objectForKey:@"city"]];
        
        NSString *loc = [responseDic objectForKey:@"loc"];
