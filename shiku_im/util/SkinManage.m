...
-(instancetype)init{
    if (self = [super init]) {
        ...
        if (current == nil) {
            [g_default setObject:[NSNumber numberWithUnsignedInteger:SkinType_Blue] forKey:SkinDictKeyIndex];
            [g_default synchronize];
            current = [NSNumber numberWithUnsignedInteger:SkinType_Blue];
        }
...
//    0x7EB7E3
    [skinList addObject:[self makeASkin:@"极简蓝" color:HEXCOLOR(0x3F94F7) index:SkinType_simple_Blue]];
    ...
    [skinList addObject:[self makeASkin:Localized(@"JXTheme_Blue") color:HEXCOLOR(0x3F94F7) index:SkinType_Blue]];//大海蓝
